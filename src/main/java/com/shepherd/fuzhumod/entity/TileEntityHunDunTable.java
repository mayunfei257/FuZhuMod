...
//	private ItemStack[] hunDunTableItemStacks = new ItemStack[3];
	...
	public void execute(EntityPlayerMP player) {
		...
		if(!world.isRemote) {
			...
			if(isItemStackEmpty(itemCrystal)) {//FenJie
				int hunDunCrystalNum = 0;
				for(int index = 1; index <= 9; index++) {
					if(!isItemStackEmpty(hunDunTableItemStacks[index]) && hunDunTableItemStacks[index].getItem() == BaseControl.itemHunDunCrystal)
						hunDunCrystalNum += hunDunTableItemStacks[index].stackSize;
				}
				...
				if(emptyStackNumOfOutputStack() >= needStackNum) {
					...
					removeItemFromInputStack(BaseControl.itemHunDunCrystal, hunDunCrystalNum);
			...
			player.addChatMessage(new ChatComponentText(I18n.format(Config.MODID + ".guiHunDunTable.error3", new Object[] {this.getClass().getSimpleName()})));
